...
require([
    ...
    var drawPoint = function (x, y, second) {
        const color = second ? [255, 255, 0] : [255, 255, 255];
        return new Graphic({
            geometry: {
                type: "point",
                longitude: x,
    ...
    function createNewPoint(coord, bearing, distance) {
    ...
    view.on("click", function (event) { // Listen for the click event
        view.hitTest(event).then(function (hitTestResults) { // Search for features where the user clicked
            if (hitTestResults.results) {
                if ($('#z1').val()) {
                    list_points.push([event.mapPoint.longitude, event.mapPoint.latitude, Number($('#z1').val())]);
                } else {
                    list_points.push([event.mapPoint.longitude, event.mapPoint.latitude]);
                }
                $('#x1').val(event.mapPoint.longitude);
                $('#y1').val(event.mapPoint.latitude);
                graphicsLayer.removeAll();
                graphicsLayer.add(drawPoint(event.mapPoint.longitude, event.mapPoint.latitude));
                if ($('#d').val() && $('#a').val()) {
                    const newPoint = createNewPoint([$('#x1').val(), $('#y1').val()], $('#a').val(), $('#d').val());
                    $('#x2').val(newPoint[0]);
                    $('#y2').val(newPoint[1]);
                    graphicsLayer.add(drawPoint(newPoint[0], newPoint[1]));
                }
                $('#points').val(JSON.stringify(list_points));
            }
        })
    });
